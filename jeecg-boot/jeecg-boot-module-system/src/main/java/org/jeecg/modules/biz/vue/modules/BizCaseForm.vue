        ...
        <a-row>
          <a-col :span="24">
            <a-form-item label="商机编号" :labelCol="labelCol" :wrapperCol="wrapperCol">
              <a-input v-decorator="['bizOpptId']" placeholder="请输入商机编号"></a-input>
            </a-form-item>
          </a-col>
          <a-col :span="24">
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/biz/bizCase/add",
          edit: "/biz/bizCase/edit",
          queryById: "/biz/bizCase/queryById"
        }
      }
    },
    computed: {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'bizOpptId','content','createBy','createTime','updateBy','updateTime','subjectId'))
        })
      },
      //渲染流程表单数据
      showFlowData(){
      ...
      popupCallback(row){
        this.form.setFieldsValue(pick(row,'bizOpptId','content','createBy','createTime','updateBy','updateTime','subjectId'))
      },
    }
  }
</script>